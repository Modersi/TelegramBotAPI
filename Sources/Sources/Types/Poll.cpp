...
Telegram::Poll::Poll(const QJsonObject& json_object) {
	...
	json_object.contains("options")					? options = QJsonArrayToQVector<PollOption>(json_object["options"].toArray())															 : options = QVector<PollOption>();
	...
	json_object.contains("explanation_entities")	? explanation_entities = QJsonArrayToQVector<MessageEntity>(json_object["explanation_entities"].toArray())								 : explanation_entities = std::nullopt;
...
QJsonObject Telegram::Poll::toObject() const {
	...
	QJsonObject poll_json_object{ {"id", id}, {"question", question}, {"options", QVectorToQJsonArray(options)}, {"total_voter_count", total_voter_count},
	...
	if (explanation_entities.has_value())	poll_json_object.insert("explanation_entities", QVectorToQJsonArray(*explanation_entities));
